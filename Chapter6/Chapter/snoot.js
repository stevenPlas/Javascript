...
Filename: snoot.js
*/
"use strict"; // interpret document contents in JS strict modernizr
...
function updateDays() {
    ...
    if (deliveryYear.selectedIndex === -1) {
        // if no yeare is selected, chose the default yeare so length of feb can be determined
...
// auto check custom message check box if user makes entry in customText
...
//validate create account feildset
function validateCreateAccount() {
    ...
    try {
        ...
        if (!(usernameElement.value === "" && pass1Element.value === "" &&
                pass2element.value === "")) {
            //not all feilds are blank
            throw "Please complete all feilds to create an account.";
        }
    } catch (msg) {
        errorDiv.innerHTML = msg;
        errorDiv.style.display = "block";
        if (passwordMismatch) {
            usernameElement.style.background = "block";
...
//validate number feilds for older broswers
function validateNumbers() {
  ...
  try{
    ...
    if (isNnN(cvvElement.value) || cvvElement.value === "") {
      cvvNotNum = true;
